document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateCalendar(userId, month, year) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            // Verificar se o usuário tem plantão nesse dia
            db.collection('plantoes')
                .where('data', '==', dateStr)
                .get()
                .then((snapshot) => {
                    if (!snapshot.empty) {
                        let plantaoMarcado = false;
                        snapshot.forEach((doc) => {
                            var plantao = doc.data();
                            if (plantao.medicoId === userId) {
                                dayCell.classList.add('user-plantao'); // Marca o dia se o usuário estiver de plantão
                                plantaoMarcado = true;
                            }
                            if (plantao.status === 'disponível') {
                                dayCell.classList.add('plantao-disponivel'); // Marca o dia se o plantão estiver disponível
                            }
                        });

                        // Garante que somente dias com plantão do usuário sejam marcados
                        if (!plantaoMarcado) {
                            dayCell.classList.remove('user-plantao');
                        }
                    }
                })
                .catch((error) => {
                            ...
                            if (medicoDoc.exists) {
                                ...
                                if (plantao.medicoId === userId) {
                                    if (plantao.status === 'confirmado') {
                                        ...
                                        anunciarBtn.classList.add('assumir-btn');  // Adiciona a classe CSS
                                        ...
                                        cancelarBtn.classList.add('apagar-btn');  // Adiciona a classe CSS
