    ...
    function generateCalendar(userId, month, year) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            db.collection('plantoes')
                ...
                .then((snapshot) => {
                    if (!snapshot.empty) {
                        console.log('Plantão encontrado para o dia:', dateStr);
                        snapshot.forEach((doc) => {
                            var plantao = doc.data();
                            if (plantao.status === 'disponível') {
                                dayCell.classList.add('plantao-disponivel-day'); // Marca o dia com plantão disponível em verde
                            } else if (plantao.status === 'confirmado') {
                                dayCell.classList.add('plantao-day'); // Marca o dia com plantão confirmado em azul
                            }
                        });
                    }
                })
                        ...
                        db.collection('users').doc(plantao.medicoId).get().then((medicoDoc) => {
                            if (medicoDoc.exists) {
                                ...
                                if (plantao.status === 'disponível' && plantao.medicoId !== userId) {
                                    ...
                                    assumirBtn.classList.add('assumir-btn');  // Adiciona a classe CSS
                                ...
                                // Exibe uma mensagem se o médico não for encontrado
                                var listItem = document.createElement('li');
                                listItem.innerText = `Local: ${plantao.local}, Horário: ${plantao.horario}, Médico: N/A, Status: ${plantao.status}`;
                                plantaoList.appendChild(listItem);
                            }
                        }).catch((error) => {
